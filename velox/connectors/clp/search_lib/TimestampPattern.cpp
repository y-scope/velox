...
std::unique_ptr<TimestampPattern[]> TimestampPattern::m_known_ts_patterns =
    nullptr;
std::mutex TimestampPattern::m_init_mutex;
...
/**
 * Converts a value to a padded string with the given length and appends it to
 * the given string
 * @param value
...
/**
 * Converts a value to a padded string with the given length and appends it to
 * the given string. Omits trailing 0.
 * @param value
 * @param padding_character
 * @param length
 * @param str
 */
static void append_padded_value_notz(
    int value,
    char padding_character,
    size_t max_length,
    string& str);
...
/**
 * Converts a padded decimal integer string with no trailing zeros (from a
 * larger string) to an integer
 * @param str String containing the numeric string
 * @param max_digits
 * @param begin_ix Start position of the numeric string
...
/*
 * To initialize m_known_ts_patterns, we first create a vector of patterns then
 * copy it to a dynamic array. This eases maintenance of the list and the cost
 * doesn't matter since it is only done once when the program starts.
 */
void TimestampPattern::init() {
  ...
  // E.g. Started POST "/api/v3/internal/allowed" for 127.0.0.1 at 2017-06-18
  // 00:20:44
  ...
  // TODO These patterns are imprecise and will prevent searching by timestamp;
  // but for now, it's no worse than not parsing a timestamp E.g. Jan 21
  // 11:56:42
  ...
  m_known_ts_patterns =
      std::make_unique<TimestampPattern[]>(m_known_ts_patterns_len);
...
TimestampPattern const* TimestampPattern::search_known_ts_patterns(
  ...
  for (size_t i = 0; i < m_known_ts_patterns_len; ++i) {
    if (m_known_ts_patterns[i].parse_timestamp(
            line, timestamp, timestamp_begin_pos, timestamp_end_pos)) {
...
bool TimestampPattern::parse_timestamp(
  ...
  for (num_spaces_found = 0;
       num_spaces_found < m_num_spaces_before_ts && line_ix < line_length;
       ++line_ix) {
    if (' ' == line[line_ix]) {
      ++num_spaces_found;
    }
    ...
    if (false == is_specifier) {
      ...
      switch (m_format[format_ix]) {
        ...
        case 'y': { // Zero-padded year in century
          ...
          if (false ==
                  convert_string_to_number(
                      line, line_ix, line_ix + cFieldLength, '0', value) ||
              value < 0 || value > 99) {
            return false;
          }
          year = value;
        ...
        case 'B': { // Month name
          ...
          for (int month_ix = 0; !match_found && month_ix < cNumMonths;
               ++month_ix) {
        ...
        case 'b': { // Abbreviated month name
          ...
          for (int month_ix = 0; !match_found && month_ix < cNumMonths;
            ...
            if (0 ==
                line.compare(line_ix, length, cAbbrevMonthNames[month_ix])) {
              month = month_ix + 1;
              match_found = true;
              line_ix += length;
            }
        ...
        case 'e': { // Space-padded day in month
          ...
          if (false ==
                  convert_string_to_number(
                      line, line_ix, line_ix + cFieldLength, ' ', value) ||
              value < 1 || value > 31) {
            return false;
          }
          date = value;
        ...
        case 'a': { // Abbreviated day of week
          bool match_found = false;
          for (int day_ix = 0; !match_found && day_ix < cNumDaysInWeek;
               ++day_ix) {
            size_t const abbrev_length = strlen(cAbbrevDaysOfWeek[day_ix]);
            if (0 ==
                line.compare(
                    line_ix, abbrev_length, cAbbrevDaysOfWeek[day_ix])) {
              match_found = true;
          ...
          // Weekday is not useful in determining absolute timestamp, so we
        ...
        case 'T': { // Zero-padded millisecond no trailing zero
          ...
          if (!convert_string_to_number_notz(
                  line, cMaxFieldLength, line_ix, new_line_ix, value) ||
              value < 0 || value > 999) {
            return false;
          }
          millisecond = value;
        ...
        case 'E': { // Millisecond-precision UNIX epoch timestamp
          ...
          // Note: this format will also accept any integer timestamp (including
          // UNIX epoch seconds and nanoseconds as well)
          if (line_ix > 0 ||
        ...
        case 'F': { // Nanosecond-precision floating-point UNIX epoch timestamp
          ...
          if (false ==
              StringUtils::convert_string_to_int64(
                  timestamp_view.substr(0, dot_position), timestamp)) {
            return false;
          }
  ...
  auto timestamp_point = date::sys_days(year_month_date) +
    ...
    if (false == is_specifier) {
      ...
      switch (m_format[format_ix]) {
        ...
        case 'E': // UNIX epoch milliseconds
          // Note: this timestamp format is required to make up the entire
...
bool operator==(TimestampPattern const& lhs, TimestampPattern const& rhs) {
  return (
      lhs.m_num_spaces_before_ts == rhs.m_num_spaces_before_ts &&
