...
std::string& QueryRunner::get_cached_decompressed_unstructured_array(
    int32_t column_id) {
...
size_t QueryRunner::fetch_next(
  ...
  while (m_cur_message < m_num_messages) {
    ...
    if (evaluate(m_expr.get(), m_schema)) {
      m_schema_reader->get_message(
          m_cur_message, num_rows_fetched, column_vectors);
      num_rows_fetched += 1;
    ...
    switch (cur_type) {
      case ExpressionType::And:
        if (false == ret || m_expression_state.top().second == cur->op_end()) {
          ...
          cur = static_cast<Expression*>(
              (m_expression_state.top().second++)->get());
    ...
    switch (
      ...
      case LiteralType::ArrayT:
        ret = evaluate_wildcard_array_filter(
            op, get_cached_decompressed_unstructured_array(column_id), literal);
        break;
      default:
        break;
    }
...
bool QueryRunner::evaluate_int_filter_core(
    FilterOperation op,
    int64_t value,
    int64_t operand) {
  switch (op) {
...
bool QueryRunner::evaluate_float_filter_core(
    FilterOperation op,
    double value,
    double operand) {
  switch (op) {
  ...
  switch (item.type()) {
    case ondemand::json_type::object: {
      ...
      if (evaluate_array_filter_object(
              nested_object, op, unresolved_tokens, cur_idx, operand)) {
        match = true;
      }
    } break;
    case ondemand::json_type::array: {
      ondemand::array nested_array = item.get_array();
      if (evaluate_array_filter_array(
              nested_array, op, unresolved_tokens, cur_idx, operand)) {
        match = true;
      }
    } break;
    ...
    case ondemand::json_type::number: {
      ...
      if (number.is_double()) {
        ...
        int64_t tmp_int;
        operand->as_int(tmp_int, op);
        // TODO: once we properly support unsigned at at least the AST level we
        // should replace this with something like operand->as_uint(tmp_uint)
    ...
    case ondemand::json_type::boolean: {
      ...
      bool tmp_bool;
      if (operand->as_bool(tmp_bool, op) &&
          eval(op, item.get_bool(), tmp_bool)) {
        match = true;
      }
    } break;
...
bool QueryRunner::evaluate_array_filter_array(
  ...
  for (ondemand::value item : array) {
    if (evaluate_array_filter_value(
            item, op, unresolved_tokens, cur_idx, operand)) {
      return true;
    }
  }
  return false;
...
bool QueryRunner::evaluate_array_filter_object(
  ...
  for (auto field : object) {
    // Note: field.key() yields the escaped JSON key, so the descriptor tokens
    // passed to search must likewise be escaped.
    ...
    return evaluate_array_filter_value(
        item, op, unresolved_tokens, cur_idx, operand);
  }
  return false;
}
...
void QueryRunner::populate_string_queries(
  ...
  if (filter != nullptr &&
    ...
    if (filter->get_column()->matches_type(LiteralType::ClpStringT)) {
      ...
      filter->get_operand()->as_clp_string(
          query_string, filter->get_operation());
      ...
      m_string_query_map.emplace(
          ...
          Grep::process_raw_query(
              m_log_dict, m_var_dict, query_string, m_ignore_case, false));
    }
    SubQuery sub_query;
    if (filter->get_column()->matches_type(LiteralType::VarStringT)) {
      std::string query_string;
      filter->get_operand()->as_var_string(
          query_string, filter->get_operation());
        ...
        if (entry != nullptr) {
          matching_vars.insert(entry->get_id());
                     ...
                     wildcard_search_dictionary_and_get_encoded_matches(
                         query_string, *m_var_dict, m_ignore_case, sub_query)) {
...
void QueryRunner::populate_searched_wildcard_columns(
    std::shared_ptr<Expression> const& expr) {
  if (expr->has_only_expression_operands()) {
    for (auto const& op : expr->get_op_list()) {
      populate_searched_wildcard_columns(
          std::static_pointer_cast<Expression>(op));
    }
    ...
    for (int32_t node : (*m_schemas)[m_schema]) {
      ...
      if (col->matches_type(node_to_literal_type(tree_node_type))) {
        ...
        if (NodeType::ClpString != tree_node_type &&
...
EvaluatedValue QueryRunner::constant_propagate(
    std::shared_ptr<Expression> const& expr) {
  if (std::dynamic_pointer_cast<OrExpr>(expr)) {
    ...
    for (auto it = expr->op_begin(); it != expr->op_end(); it++) {
      ...
      if (ret == EvaluatedValue::True) {
        return expr->is_inverted() ? EvaluatedValue::False
                                   : EvaluatedValue::True;
    ...
    for (auto it = expr->op_begin(); it != expr->op_end(); it++) {
      ...
      if (ret == EvaluatedValue::False) {
        return expr->is_inverted() ? EvaluatedValue::True
    ...
    if ((filter->get_operation() == FilterOperation::EXISTS ||
      ...
      constexpr LiteralTypeBitmask other_types = LiteralType::ArrayT |
          cIntegralTypes | LiteralType::NullT | LiteralType::BooleanT |
          LiteralType::EpochDateT;
      bool has_other = wildcard->matches_any(other_types);
      std::string filter_string;
      bool valid = filter->get_operand()->as_var_string(
                       filter_string, filter->get_operation()) ||
          filter->get_operand()->as_clp_string(
              filter_string, filter->get_operation());
      if (false == valid) {
        // FIXME: throw
      ...
      if (filter->get_column()->matches_type(LiteralType::VarStringT)) {
        m_expr_var_match_map[expr.get()] =
            &m_string_var_match_map.at(filter_string);
      ...
      if (filter->get_operation() == FilterOperation::EQ) {
        ...
        if (has_clp_string && !matches_clp_string ||
          ...
          return filter->is_inverted() ? EvaluatedValue::False
            ...
            false == has_clp_string && false == has_var_string && !has_other) {
          return EvaluatedValue::False;
        }
      } else {
        // FIXME: throw
...
bool QueryRunner::evaluate_epoch_date_filter(
  ...
  return evaluate_int_filter_core(
      op, reader->get_encoded_time(m_cur_message), op_value);
}
