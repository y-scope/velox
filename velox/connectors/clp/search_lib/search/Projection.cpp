...
void Projection::add_column(std::shared_ptr<ColumnDescriptor> column, ColumnType node_type) {
  if (column->is_unresolved_descriptor()) {
    throw OperationFailed(ErrorCodeBadParam, __FILE__, __LINE__);
  }
  if (ProjectionMode::ReturnAllColumns == m_projection_mode) {
  ...
  switch (node_type) {
    case ColumnType::String:
      column->set_matching_types(LiteralType::ClpStringT | LiteralType::VarStringT | LiteralType::EpochDateT);
      break;
