...
public:
     ...
     * @throws OperationFailed if `column` is identical to a previously added column
     */
    void add_column(std::shared_ptr<ColumnDescriptor> column, ColumnType node_type);
     ...
     */
    std::vector<std::vector<int32_t>> const& get_matching_nodes_list() const { return m_matching_nodes_list; }
