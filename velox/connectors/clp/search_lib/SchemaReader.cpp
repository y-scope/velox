  ...
  if (m_timestamp_column->get_type() == NodeType::DateString) {
    ...
    m_get_timestamp = [this]() {
      return std::get<int64_t>(
          static_cast<Int64ColumnReader*>(m_timestamp_column)
...
void SchemaReader::load(
  ...
  auto error = decompressor.try_read_exact_length(
      m_table_buffer.get(), uncompressed_size);
  ...
  while (m_json_serializer.get_next_op(op)) {
    switch (op) {
      ...
      case JsonSerializer::Op::AddIntField: {
        ...
        m_json_serializer.append_value(std::to_string(
            std::get<int64_t>(column->extract_value(m_cur_message))));
        break;
      }
      ...
      case JsonSerializer::Op::AddFloatField: {
        ...
        m_json_serializer.append_key(name);
        m_json_serializer.append_value(std::to_string(
            std::get<double>(column->extract_value(m_cur_message))));
        break;
      }
      ...
      case JsonSerializer::Op::AddStringField: {
        ...
        auto const& name =
            m_global_schema_tree->get_node(column->get_id()).get_key_name();
        m_json_serializer.append_key(name);
        m_json_serializer.append_value_from_column_with_quotes(
            column, m_cur_message);
        break;
      }
...
void SchemaReader::initialize_query_runner(
  ...
  for (const auto& node_ids : matching_nodes_list) {
    ...
    for (const auto node_id : node_ids) {
      auto column_it = m_column_map.find(node_id);
      if (column_it != m_column_map.end()) {
        m_projected_columns.push_back(column_it->second);
        m_projected_types.push_back(m_global_schema_tree->get_node(node_id).get_type());
        found_reader = true;
        break;
      }
    }
...
void SchemaReader::mark_unordered_object(
  ...
  m_global_id_to_unordered_object.emplace(
      mst_subtree_root, std::make_pair(column_reader_start, schema));
}
...
void SchemaReader::find_intersection_and_fix_brackets(
  ...
  // The loop above ends when the parent of next node and cur node matches. When
  // these two nodes have the same parent but are different nodes we need to
  // close the last bracket for the previous node, and add the first key for
  // next node.
  ...
  for (auto it = path_to_intersection.rbegin();
       it != path_to_intersection.rend();
       ++it) {
    ...
    if (NodeType::Object == node.get_type()) {
      ...
      m_json_serializer.add_op(
          no_name ? JsonSerializer::Op::BeginUnnamedArray
    ...
    if (Schema::schema_entry_is_unordered_object(global_column_id)) {
      ...
      if (NodeType::StructuredArray == type) {
        ...
        column_idx = generate_structured_array_template(
            sub_array_root, column_idx, sub_object_schema);
        ...
        column_idx = generate_structured_object_template(
            object_root, column_idx, sub_object_schema);
        m_json_serializer.add_op(JsonSerializer::Op::EndObject);
      }
      i += length;
    } else {
      ...
      switch (node.get_type()) {
        case NodeType::Object: {
          find_intersection_and_fix_brackets(
              array_root, global_column_id, path_to_intersection);
...
size_t SchemaReader::generate_structured_object_template(
  ...
  for (size_t i = 0; i < schema.size(); ++i) {
    int32_t global_column_id = schema[i];
    if (Schema::schema_entry_is_unordered_object(global_column_id)) {
      // It should only be possible to encounter arrays inside of structured
      // objects
      size_t array_length =
          Schema::get_unordered_object_length(global_column_id);
      auto array_schema = schema.subspan(i + 1, array_length);
      // we can guarantee that the last array we hit on the path to object root
      // must be the right one because otherwise we'd be inside the structured
      // array generator
      int32_t array_root =
          m_global_schema_tree->find_matching_subtree_root_in_subtree(
              object_root,
      ...
      column_idx = generate_structured_array_template(
          array_root, column_idx, array_schema);
      m_json_serializer.add_op(JsonSerializer::Op::EndArray);
      i += array_length;
      // root is parent of the array object since we close the array bracket
      // above
...
void SchemaReader::initialize_serializer() {
  ...
  // TODO: this code will have to change once we allow mixing log lines parsed
  ...
  for (int32_t child_id : children_ids) {
    ...
    switch (child_node.get_type()) {
      ...
      case NodeType::StructuredArray: {
        ...
        m_json_serializer.add_special_key(key);
        auto structured_it =
            m_global_id_to_unordered_object.find(child_global_id);
        if (m_global_id_to_unordered_object.end() != structured_it) {
          ...
          generate_structured_array_template(
              child_global_id, column_start, structured_schema);
        }
        m_json_serializer.add_op(JsonSerializer::Op::EndArray);
        break;
      }
