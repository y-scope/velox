...
class FilterClass {
 public:
  ...
  virtual void init(
      SchemaReader* reader,
      int32_t schema_id,
      std::vector<BaseColumnReader*> const& column_readers) = 0;
...
class SchemaReader {
 public:
  ...
  /**
   * Resets the contents of this SchemaReader and prepares it to become a
   * SchemaReader with a new schema id, schema tree, and other parameters. After
   * this call the SchemaReader is prepared to accept append_column calls for
   * the new schema.
   *
   * @param schema_tree
   * @param projection
  ...
  /**
   * Populates the column vectors with the values of the message at the given
   * index
   * @param message_index
   * @param vector_index
   ...
   * @param query_runner
   */
  void initialize_query_runner(
      std::shared_ptr<search::QueryRunner> query_runner);
  ...
  int32_t get_schema_id() const {
    return m_schema_id;
  }

  /**
   * @param schema
   * @return the first column ID found in the given schema, or -1 if the schema
   * contains no columns
   */
   ...
   */
  epochtime_t get_next_timestamp() const {
    return m_get_timestamp();
  }

  /**
   * @return true if all records in this table have been iterated over, false
   * otherwise
   */
  bool done() const {
    return m_cur_message >= m_num_messages;
  }

  /**
   * @return the number of messages in the table
   */
  uint64_t get_num_messages() const {
    return m_num_messages;
  }

 private:
  /**
   * Merges the current local schema tree with the section of the global schema
   * tree corresponding to the path from the root of the global schema tree to
   * the node matching the global MPT node id passed to this function.
   * @param global_id
   */
   ...
   * @param id
   * @param column_start the index of the first reader in m_columns belonging to
   * this array
   * @param schema
   * @return the index of the next reader in m_columns after those consumed by
   * this array
   */
  size_t generate_structured_array_template(
      int32_t id,
      size_t column_start,
      std::span<int32_t> schema);
   ...
   * this object
   */
  size_t generate_structured_object_template(
      int32_t id,
      size_t column_start,
      std::span<int32_t> schema);

  /**
   * Finds the common root of the subtree containing cur_root and next_root, and
   * adds brackets and keys to m_json_serializer as necessary so that the json
   * object is correct between the previous field which is a child of cur_root,
   * and the next field which is a child of next_root.
   *
   * For example for the object {"a": {"b": "c"}, "d": {"e": {"f": "g"}}} after
   * appending "b" cur_root would be "a", and next_root would be "e". (since it
   * is the parent of the next field "f"). The current state of the object would
   * look like "a":{"b":"c" -- to prepare for "f" we would add },"d":{"e":{ or
   * in other words close one bracket, add "d" and open bracket, add "e" and
   * open bracket. After adding field "f" the current root is "e", and the next
   * root is the original object which is the parent of "a" so we add }}.
   *
   * This works by tracing the path between both cur_root and next_root to their
   * nearest common ancestor. For every step cur_root takes towards this common
   * ancestor we must close a bracket, and for every step on the path from
   * next_root a key must be added and a bracket must be opened. The parameter
   * `path_to_intersection` is used as a buffer to store the path from next_root
   * to this intersection so that the keys can be added to m_json_serializer in
   * the correct order.
   * @param cur_root
   * @param next_root
  ...
  std::map<int32_t, std::pair<size_t, std::span<int32_t>>>
